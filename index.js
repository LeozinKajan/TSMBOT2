...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Boa sorte ae para vc @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			body = (type === 'conversação' && mek.message.conversation.startsWith(prefix)) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption.startsWith(prefix) ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption.startsWith(prefix) ? mek.message.videoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text.startsWith(prefix) ? mek.message.extendedTextMessage.text : ''
			budy = (type === 'conversação') ? mek.message.conversation : (type === 'extendedTextMessage') ? mek.message.extendedTextMessage.text : ''
			...
			mess = {
				...
				success: '✔️ Pronto ✔️',
				error: {
					stick: '❌ Ocorreu um erro ao converter a imagem em um adesivo ❌',
					Iv: '❌ Link pode não estar certo ❌'
				},
				only: {
			...
			const ownerNumber = ["351965098553@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer, (err) => {
								if (err) return reply('Falhou, Há um erro, por favor, tente novamente mais tarde.')
							})
				...
				case 'gtts':
					...
					if (args.length < 2) return client.sendMessage(from, 'TCadê a ext, tio', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Lembrar! Amo sua lolita'})
					})
					break
				case 'nsfwloli':
				...
				case 'tulis':
					...
					break
				case 'url2img':
					tipelist = ['Area de trabalho','tablet','Móvel']
					if (args.length < 1) return reply('Qual é o tipo hum?')
					if (!tipelist.includes(args[0])) return reply('Tipo desktop|tablet|Móvel')
				...
				case 'simih':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Hmmmm, 2, pula o 3 e vem de 4')
					if (Number(args[0]) === 1) {
				...
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Hmmm, 2 pula o 3 e vem de 4')
					if (Number(args[0]) === 1) {
